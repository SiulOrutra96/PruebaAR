...
var _ref4 = (function directionToWorldCoordsFunction() {
    ...
    return {
       ...
       */
      directionToWorldCoords: function directionToWorldCoords(
        THREE,
        aframeCamera,
        camera,
        _ref5,
        ...
        // A line from the camera position toward (and through) the plane
        var newPosition = rayPlaneIntersection(
...
function dragItem(THREE, element, offset, camera, depth, mouseInfo) {
  ...
  function onMouseMove(_ref7) {
    ...
    var direction = screenCoordsToDirection(THREE, camera, {
      x: clientX,
      y: clientY,
    });

    var _directionToWorldCoor = directionToWorldCoords(
        THREE,
        camera,
    ...
    console.log('lastMouseInfo: ', lastMouseInfo);
    console.log('direction: ', direction);
    console.log('_directionToWorldCoor: ', _directionToWorldCoor);
    ...
    rotationDiff = rotationQuaternion.multiply(
      threeCamera.getWorldQuaternion()
    );
  ...
  function onCameraChange(_ref9) {
    ...
    if (
      ...
      !(0, _deepEqual2.default)(detail.oldData, detail.newData)
    ) {
      ...
      function onMouseDown(_ref12) {
        ...
        if (element) {
          (function () {
            ...
            var removeDragItemListeners = dragItem(
              THREE,
              element,
              offset,
              camera,
...
function aframeDraggableComponent() {
  var componentName =
